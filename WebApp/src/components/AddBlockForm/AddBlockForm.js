...
class AddBlockForm extends Component {
  state = {
    ...
    loading: false,
    blockColor:
      this.props.course.blockColor == null
        ? toMaterialStyle(
            this.props.course.courseCode,
            this.props.course.blockColor
          ).backgroundColor
  ...
  fetchCourseBlock = (courseName) => {
    ...
    for (var i = 0; i < this.state.suggestions.length; i++) {
      if (
        this.state.suggestions[i].courseName == courseName.split(" (")[0] &&
        this.state.suggestions[i].courseCode ==
          courseName.split(" (")[1].split(") ")[0] &&
        this.state.suggestions[i].yearOfStudy + ".r" ==
          courseName.split(") ")[1].split(",")[0] &&
        this.cutStudyType(this.state.suggestions[i].studyType) ==
          courseName.split(") ")[1].split(",")[1]
      ) {
        j = i;
      }
    }
    ...
    axios
      .get(
        `/api/timetable/getCourseBlock/${this.state.suggestions[j].id}/${startBlock}/${this.state.day}`
      )
      .then(({ data }) => {
          ...
          <DialogContent>
            <FlexBox>
              <Tabs value={tabValue} onChange={this.handleChangeTab}>
                <Tab sx={{ width: "50%" }} label="Predmety" />
                <Tab sx={{ width: "50%" }} label="Udalosti" />
              </Tabs>
